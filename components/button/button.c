...
static void button_update_task(void* arg) {
    ...
    for (;;) {
        ...
        if (gpio_get_level(btn->pin_io) == btn->pressed_value) {
            btn->pressed = 1;
...
uint8_t button_isPressed(button_t* button) {
    return gpio_get_level(button->pin_io) == button->pressed_value;
}
...
button_t* button_attch(uint16_t gpio_pin, uint8_t press_value, uint16_t filter_time) {
    ...
    gpio_pad_select_gpio(button->pin_io);
    gpio_set_direction(button->pin_io, GPIO_MODE_INPUT);

    gpio_set_intr_type(button->pin_io, GPIO_INTR_ANYEDGE);
    gpio_isr_handler_add(button->pin_io, gpio_isr_handler, button);
